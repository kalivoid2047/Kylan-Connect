...
- ✅ WiFi Direct (Android, opt-in, experimental). Deliberately does NOT use the `wifi_direct_plugin` package's own bundled messaging protocol (low-adoption package; its Kotlin implementation runs a separate ad-hoc socket/chunking layer we don't trust for delivery). Instead `WifiDirectService` uses it only to request runtime permissions (`ACCESS_FINE_LOCATION`/`NEARBY_WIFI_DEVICES`, which the plugin does not request itself), discover/connect to a peer, and detect when a link forms. Once connected, Android exposes WiFi Direct as an ordinary IP interface (`p2p0`, ~192.168.49.x); `DiscoveryService.refreshInterfacesAndBroadcast()` (new) re-scans local interfaces and immediately broadcasts, so the existing UDP discovery → TOFU pinning → AES-GCM messaging pipeline finds the peer with zero protocol changes. UI: `WifiDirectScreen` (Settings → "WiFi Direct (Android, experimental)") — enable, see nearby devices, tap to connect. Android manifest overrides the plugin's `required="true"` WiFi Direct hardware feature to `false` (opt-in shouldn't make the app uninstallable on devices without it) and strips permissions the plugin declares for its unused bundled feature (CAMERA, storage/media — verified via other installed plugins' manifests that nothing else in the app needs them). Bumped `file_picker` 8→10 (forced by the plugin's dependency; verified no breaking API changes at that range). **Unverified**: real-device WiFi Direct pairing/group-formation behavior — this is one of the most OEM-fragile native Android APIs and there is no way to test it without physical hardware in this environment; only Gradle/manifest-merge compilation was verified.
  - On-device testing found permissions granted but `discoverPeers()` returning zero peers, with no error surfaced. Fixed in two steps: `_enable()` now shows a snackbar when the native `startDiscovery()` call itself fails (commit `b4c8f3c`); separately, `WifiDirectService.startDiscovery()` now checks `Permission.location.serviceStatus` — the system Location Services toggle, distinct from the app's own already-granted location permission — before calling into the native plugin, since on many OEMs `discoverPeers()` still calls `onSuccess` with Location Services off and just silently never reports any peers (commit `00833c5`). Still unverified beyond this: whether peers are found once Location Services is confirmed on and both devices are discovering simultaneously.
- Mesh relay/store-and-forward for emergency scenarios
